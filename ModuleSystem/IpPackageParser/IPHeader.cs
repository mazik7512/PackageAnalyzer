    ...
    public class IPHeader
    {
        ...
        public bool SetData(ref byte[] array, ref int index)
        {
            ...
            if (index < array.Length)
            {
                try
                {
                    for (i = index; i < array.Length - ipMarker.Length /*- 224*/; i++)
                    {
                        //Проверка на ip
                        if (array[i] == ipMarker[0] && array[i + 1] == ipMarker[1] && array[i + 2] == ipMarker[2] && array[i + 3] == ipMarker[3])
                        {
                            for (int j = 4; j < ipMarker.Length; j++)
                            {
